...
function StatisticTile({ 
  ...
  return (
    <div className={`statistics-tile statistics-tile--${variant} ${className}`}>
      <div className="statistics-tile__header">
        <div className="statistics-tile__icon">
          {icon}
        </div>
        <span className="statistics-tile__label">{label}</span>
      </div>
      <div className="statistics-tile__content">
        <div className={`statistics-tile__value ${valueColor}`}>
          {value}
        </div>
...
function StatisticsTiles({ items, viewMode }) {
  const counts = build_recommendation_counts(items);
  const total_issues = items.length;

  const schema_score = calculate_schema_score(counts, total_issues);
  const score_color = get_score_color(schema_score);
  const score_status = get_score_status(schema_score)
    ...
    <div className="statistics-tiles">
      <div className="statistics-tiles__grid">
        ...
        <StatisticTile
          icon={icons.score}
          label="Schema Score"
          value={`${schema_score}/100`}
          subtitle={score_status}
          valueColor={score_color}
          variant="score"
        />
        ...
        <StatisticTile
          ...
          label="Total Issues"
          value={total_issues}
          subtitle={viewMode === 'all' ? 'All Tables' : 'This Table'}
          variant="total"
        />
...
export default function EnhancedRecommendationsSidebar({
    ...
    const processedItems = useMemo(() => {
        ...
        // Apply type filter
        if (selectedFilter) {
            filtered = filter_recommendations_by_type(filtered, selectedFilter);
        }

        // Apply sorting
        if (sortBy === 'priority') {
            filtered = sort_recommendations_by_priority(filtered);
        } else if (sortBy === 'type') {
